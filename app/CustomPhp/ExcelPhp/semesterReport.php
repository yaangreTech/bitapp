...
function SemesterReport($headers = [], $student_data = [], $className, $semesterNumber, $academicYear, $saveInFolder = ''): void
{
    ...
    $sheet->Rotate('E2:' . $alphabet[START_COL_Se + $total_numbers_of_rows_to_rotate - 1] . '2', 90);
    //centers vertically cells text of headers
    $sheet->SetCenter('E2:' . $alphabet[START_COL_Se + $total_numbers_of_rows_to_rotate - 1] . '2', false, true);
    //sets text to bold: cells of headers
    $sheet->SetCellsToBold('E2:' . $alphabet[START_COL_Se + $total_numbers_of_rows_to_rotate - 1] . '2');
    //sets text to italic: cells of modulus
    $sheet->SetCellsToItalic('E2:' . $alphabet[START_COL_Se + $total_numbers_of_rows_to_set_in_italic - 1] . '2');
    //sets text to bold: cells of name, last name and number of credits
    $sheet->SetCellsToBold('A3:' . $alphabet[START_COL_Se + $total_numbers_of_rows_to_rotate - 1] . '3');

    //adds and formats headers text
    foreach ($finalHeaders as $key => $value) {
        $key_index = array_search($key, $keys);
        $sheet->Write($alphabet[$key_index + START_COL_Se] . '2', $key);
        $sheet->Write($alphabet[$key_index + START_COL_Se] . '3', $value);
        //center horizontally the 2nd row
        $sheet->SetCenter($alphabet[$key_index + START_COL_Se] . '3', false, true);

        //sets default the background color for other extra data => row for credits
        $sheet->SetColor($alphabet[$key_index + START_COL_Se] . '3', 'ffd9d9d9');

        //if the current element is in the list of modules
        if (in_array($value, array_values($modules))) {
            //sets the color for the module
            $sheet->SetColor($alphabet[$key_index + START_COL_Se] . '2', SKY_BLUE);
            //sets the color for the number of credits
            $sheet->SetColor($alphabet[$key_index + START_COL_Se] . '3', SKY_BLUE);
        }
        ...
        if (in_array($key, ['Total of Credits', 'Final Average'])) {
            ...
            $sheet->SetColor($alphabet[$key_index + START_COL_Se] . '3', 'ffc4bd97');
        }
    }
