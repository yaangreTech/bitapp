...
class SimpleEdit {
    ...
    buildPattern() {
        ...
        this.pattern = $(
            `<${this.node} class="simpleedit">
                 <input max=20 min=0 style="height:26px" type="number" value="${this.text}">
                 ${this.submitButton}
                 ${this.abortButton}
             </${this.node}>`
        )
    ...
    submit() {
        ...
        console.log(text);
        var data_pk = $(this.element).attr('data-pk')
            // If url is given
            // const url = this.url || $(this.element).attr('data-url')
        var url = false;
        var refss = data_pk.split('_')
        console.log(refss);
        if (refss.length < 2) {
            url = '/marks_modulus/update_marks_modulus_marks_of/' + refss[0];
        } else {
            url = '/marks_modulus/store_marks_modulus_marks_of/' + refss[0] + '/' + refss[1];
        }

        console.log(data_pk);
        if (url) {
            // Collect data
            let data = { mark_value: text }
